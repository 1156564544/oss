...
func SearchLatestVersion(name string) (meta Metadata, e error) {
	url := fmt.Sprintf("http://%s/metadata/_search?q=name:%s&size=1&sort=version:desc",
		"localhost:9200", url.PathEscape(name))
	r, e := http.Get(url)
	if e != nil {
		return
	}
...
func SearchVersionStatus(min_doc_count int) ([]Bucket, error) {
	client := http.Client{}
	url := fmt.Sprintf("http://%s/metadata/_search", "localhost:9200")
	body := fmt.Sprintf(`
        {
          "size": 0,
          "aggs": {
...
func HasHash(hash string) (bool, error) {
	url := fmt.Sprintf("http://%s/metadata/_search?q=hash:%s&size=0", "localhost:9200", hash)
	r, e := http.Get(url)
	if e != nil {
		return false, e
	}
